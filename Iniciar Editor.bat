...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$d=[Environment]::GetFolderPath('DesktopDirectory'); $p=Join-Path $d 'Ponto B - Video Editor.lnk'; if(Test-Path $p){exit 0}; $ws=New-Object -ComObject WScript.Shell; $sc=$ws.CreateShortcut($p); $sc.TargetPath='%BAT_PATH%'; $sc.WorkingDirectory='%PROJECT_DIR%'; if(Test-Path '%ICON_PATH%'){$sc.IconLocation='%ICON_PATH%,0'}; $sc.Description='Ponto B - Editor de Reels'; $sc.Save(); Write-Host ''; Write-Host ('  Atalho criado em ' + $p) -ForegroundColor Green; Write-Host ''"
